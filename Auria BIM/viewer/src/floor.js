...
//
// IMPORTANTE: acumula os volumes ASSINADOS de todos os submeshes juntos e só
// ao final toma o módulo. Isso permite que faces abertas (topo, fundo, laterais
// armazenadas em meshes separadas) se cancelem corretamente — como se formassem
// uma única malha fechada. Chamar Math.abs por submesh (bug anterior) resultava
// em somar "cones" independentes e inflava o volume em até 10-15×.
function _computeMeshVolume(entity) {
  if (!entity) return 0;
  const meshes = entity.meshes;
  if (meshes?.length) {
    let signed = 0; // acumulador com sinal único para todos os submeshes
    let hasData = false;
    for (const mesh of meshes) {
      ...
      for (let k = 0; k < idx.length; k += 3) {
        ...
        signed += (pos[a]*(pos[b+1]*pos[c+2] - pos[b+2]*pos[c+1])
                 + pos[b]*(pos[c+1]*pos[a+2] - pos[c+2]*pos[a+1])
                 + pos[c]*(pos[a+1]*pos[b+2] - pos[a+2]*pos[b+1])) / 6;
      }
    }
    // Módulo apenas uma vez, no final — XKT em metros → resultado em m³
    if (hasData) return Math.abs(signed);
  }
  return 0; // sem geometria acessível → excluído do painel (não usa AABB: muito impreciso)
}
...
function getFck(mo) {
  // Percorre o próprio objeto e sobe na hierarquia pai (decomposição IFC).
  // Objetos filhos de lajes/vigas herdam geometria mas nem sempre os property sets.
  for (let node = mo; node; node = node.parent) {
    // Não subir além do pavimento
    if (node !== mo && /IfcBuildingStorey|IfcBuilding|IfcSite|IfcProject/i.test(node.type)) break;

    let fallback = null;
    for (const ps of (node.propertySets || [])) {
      for (const p of (ps.properties || [])) {
        const raw  = typeof p.value === "object" ? p.value?.value : p.value;
        ...
        // ── 2. Propriedade com nome relacionado a resistência ──
        ...
        // ── 3. Varredura ampla: valor "C25"…"C90" ──
        if (!fallback) {
