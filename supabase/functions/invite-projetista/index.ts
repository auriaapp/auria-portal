...
serve(async (req) => {
  ...
  try {
    ...
    const { email, nome, redirect_to, empresa_nome, redefinir } = await req.json();
    ...
    //  Conta COM senha só recebe link novo se quem chamou confirmar. Sem o
    //  `redefinir`, a resposta abaixo é o que a tela usa para PERGUNTAR — e só
    //  então chama de novo com a confirmação. Antes isto era um beco: o alerta
    //  mandava a outra pessoa usar "Esqueci minha senha" e a coordenação não
    //  tinha ação nenhuma a partir dali.
    const redefinindo = !!(existente && temSenha && redefinir);
    if (existente && temSenha && !redefinir) {
      return j({
    ...
    //  Na redefinição a conta JÁ está confirmada, e aí generateLink('invite')
    //  falha — mas ele não faz falta: o link do Supabase nasce no resgate, e o
    //  convite-resgatar já cai para 'recovery' nesse caso (ativar_conta.html
    //  trata, é o IS_RECOVERY). Aqui só precisamos do id, que já temos.
    let linkData: { user?: { id?: string }; properties?: { hashed_token?: string } } | null = null;
    if (!redefinindo) {
      ...
      linkData = g.data as typeof linkData;
    }
    ...
    const _tk = await criarConviteToken(admin, { email, nome, papel: "projetista",
      user_id: linkData?.user?.id ?? existente?.id ?? null, empresa_nome, criado_por: cu?.user?.id ?? null });
    ...
    //  Na redefinição NÃO se toca no perfil: gravar role:'projetista' por cima
    //  de uma conta em uso rebaixaria quem também é analista, por exemplo. Aqui
    //  o pedido é trocar a senha, nada mais.
    if (newUserId && !redefinindo) {
    ...
    const _quem     = empresa_nome ? "a equipe da <b>" + empresa_nome + "</b>" : "a equipe de coordenação";
    const _quemTxt  = empresa_nome ? "A equipe da " + empresa_nome : "A equipe de coordenação";
    const _assunto  = redefinindo ? "Redefinir seu acesso ao Auria"
                                  : "Convite para o Auria — Painel do Projetista";
    const _chapeu   = redefinindo ? "Acesso ao Auria" : "Bem-vindo à Auria";
    const _cabeca   = redefinindo ? "Defina uma nova senha"
                                  : "Você foi convidado(a) como projetista";
    const _rotulo   = redefinindo ? "Definir nova senha" : "Definir minha senha e acessar";
    const _corpo    = redefinindo
      ? `Olá${nome ? " " + nome : ""}, ${_quem} pediu a redefinição da sua senha no Auria.
         <b>Sua senha atual continua valendo</b> até você abrir o link abaixo e escolher uma nova.`
      : `Olá${nome ? " " + nome : ""}, ${_quem}
         convidou você para acompanhar seus <b>contratos, faturas e apontamentos</b> no <b>Painel do Projetista</b> do Auria.`;
    const _corpoTxt = redefinindo
      ? `${_quemTxt} pediu a redefinição da sua senha no Auria. Sua senha atual continua valendo até você abrir o link abaixo e escolher uma nova.`
      : `${_quemTxt} convidou você para acompanhar seus contratos, faturas e apontamentos no Painel do Projetista do Auria.`;
    const html = `
      <div style="font-family:'Segoe UI',Arial,sans-serif;max-width:520px;margin:auto;color:#1A2F4A;padding:8px">
        ...
        <p style="font-size:12px;letter-spacing:.08em;color:#94A3B8;text-transform:uppercase;margin:0 0 6px">${_chapeu}</p>
        <h2 style="color:#1A2F4A;font-size:20px;margin:0 0 12px">${_cabeca}</h2>
        <p style="line-height:1.6">${_corpo}</p>
...
https://auria.solutions`,
    ...
    return j({ success: true, reenvio, redefinido: redefinindo, user_id: newUserId });
  } catch (err) {
